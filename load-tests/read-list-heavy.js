import http from 'k6/http';
import { check } from 'k6';
import { Trend, Counter } from 'k6/metrics';
import { pickListFilter } from './lib/pool.js';

const BASE_URL = __ENV.BASE_URL || 'http://localhost:3000';
const listLatency = new Trend('product_list_latency', true);
const errors = new Counter('custom_errors');

export const options = {
  scenarios: {
    read_list_heavy: {
      executor: 'constant-vus',
      vus: Number(__ENV.VUS || 50),
      duration: __ENV.DURATION || '40s',
    },
  },
  thresholds: {
    'product_list_latency': ['p(95)<1500'],
  },
  summaryTrendStats: ['avg', 'min', 'med', 'p(90)', 'p(95)', 'p(99)', 'max'],
};

export default function () {
  const filter = pickListFilter();
  const url = `${BASE_URL}/products` +
    `?category=${filter.category}` +
    `&minPrice=${filter.minPrice}` +
    `&maxPrice=${filter.maxPrice}` +
    `&sort=rating&order=desc&pageSize=20`;

  const res = http.get(url);
  listLatency.add(res.timings.duration);

  const ok = check(res, {
    'status 200': (r) => r.status === 200,
  });
  if (!ok) errors.add(1);
}

export function handleSummary(data) {
  const id = __ENV.RUN_ID || 'read-list-heavy';
  return {
    [`load-tests/results/${id}.summary.json`]: JSON.stringify(data, null, 2),
    stdout: textSummary(data),
  };
}

function textSummary(data) {
  const m = data.metrics;
  const v = m.product_list_latency?.values ?? {};
  const lines = [
    '',
    `  product_list_latency p50: ${(v.med ?? 0).toFixed(2)}ms`,
    `  product_list_latency p95: ${(v['p(95)'] ?? 0).toFixed(2)}ms`,
    `  product_list_latency p99: ${(v['p(99)'] ?? 0).toFixed(2)}ms`,
    `  http_reqs:                ${(m.http_reqs?.values?.count ?? 0)} (${(m.http_reqs?.values?.rate ?? 0).toFixed(1)}/s)`,
    `  http_req_failed:          ${((m.http_req_failed?.values?.rate ?? 0) * 100).toFixed(2)}%`,
    '',
  ];
  return lines.join('\n');
}